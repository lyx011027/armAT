...
def processDimm(id, q, dimmList, leadTime):
    for dimm in dimmList:
        ...
        if UEDf.shape[0] != 0:
            UEFlag = True
            firstUER = UEDf.loc[0, 'record_datetime']
        CEDf = df[(df['record_datetime'] <  firstUER)].reset_index(drop=True)
        if CEDf.shape[0] == 0:
            continue
        
        # firstCE, lastCE = CEDf.loc[0, 'record_datetime'], CEDf.loc[CEDf.shape[0] - 1, 'record_datetime']
        ...
        lastCE =CEDf.loc[0, 'record_datetime']
        accumulateCE = 0
        timeList = []
        adjDqLsit = []
        MaxDqDistanceLsit = []
